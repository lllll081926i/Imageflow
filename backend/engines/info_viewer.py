...
class InfoViewer:
    ...
    def _extract_svg_attribute(self, svg_tag, name):
        return extract_svg_attribute(svg_tag, name)
    ...
    def _fill_svg_info_from_text(self, text, info, extra, details):
        ...
        view_box = self._extract_svg_attribute(svg_tag, "viewBox") or self._extract_svg_attribute(svg_tag, "viewbox")
        self._apply_svg_dimensions(width_raw, height_raw, view_box, info, details)

        parsed_size = parse_svg_intrinsic_size_from_text(text)
        if parsed_size:
            info["width"], info["height"] = parsed_size
...
def process(input_data):
    try:
        ...
        if os.getenv("IMAGEFLOW_DEBUG_TRACEBACK") == "1":
