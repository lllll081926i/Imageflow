...
class GIFTool:
    ...
    def _normalize_gif_delay_ms(self, duration_ms, minimum_ms=GIF_MIN_DELAY_MS):
        try:
            value = float(duration_ms)
        except (TypeError, ValueError):
            value = float(minimum_ms)
        if value <= 0:
            value = float(minimum_ms)
        normalized = int(round(value / GIF_DELAY_UNIT_MS) * GIF_DELAY_UNIT_MS)
        return max(int(minimum_ms), normalized)

    def _retime_gif_frames(self, frames, durations, factor, disposals=None):
        if not frames:
            raise ValueError("No GIF frames to retime")

        adjusted = [max(1.0, float(duration) / factor) for duration in durations]
        if factor <= 1.0:
            normalized = [self._normalize_gif_delay_ms(duration) for duration in adjusted]
            if disposals is None:
                return frames, normalized
            return frames, normalized, list(disposals)

        retimed_frames = []
        retimed_durations = []
        retimed_disposals = [] if disposals is not None else None
        pending_duration = 0.0
        pending_frame = None
        pending_disposal = 0

        for index, (frame, duration) in enumerate(zip(frames, adjusted)):
            pending_duration += duration
            pending_frame = frame
            if disposals is not None:
                pending_disposal = disposals[index]
            if pending_duration + 1e-6 < GIF_MIN_DELAY_MS:
                continue
            retimed_frames.append(pending_frame)
            retimed_durations.append(self._normalize_gif_delay_ms(pending_duration))
            if retimed_disposals is not None:
                retimed_disposals.append(pending_disposal)
            pending_duration = 0.0
            pending_frame = None
            pending_disposal = 0

        if pending_frame is not None:
            if retimed_durations:
                retimed_durations[-1] = self._normalize_gif_delay_ms(retimed_durations[-1] + pending_duration)
                if retimed_disposals is not None:
                    retimed_disposals[-1] = pending_disposal or retimed_disposals[-1]
            else:
                retimed_frames.append(pending_frame)
                retimed_durations.append(self._normalize_gif_delay_ms(pending_duration))
                if retimed_disposals is not None:
        ...
        final_frames = retimed_frames or [frames[-1]]
        final_durations = retimed_durations or [GIF_MIN_DELAY_MS]
        if retimed_disposals is None:
            return final_frames, final_durations
        final_disposals = retimed_disposals or [disposals[-1] if disposals else 0]
        return final_frames, final_durations, final_disposals
